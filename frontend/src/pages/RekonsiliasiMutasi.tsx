...
const SearchableRkatDropdown: React.FC<SearchableRkatDropdownProps> = ({
  ...
  onChange,
  options,
  placeholder = "Cari kegiatan, program, kode, atau keterangan...",
  allowEmpty = true,
          ...
          <div className="overflow-y-auto custom-scrollbar p-1.5 space-y-1 max-h-64 divide-y divide-slate-100">
            ...
            {filteredOptions.length === 0 ? (
              <div className="p-5 text-center text-slate-400 space-y-1">
                ...
                <p className="text-[10px]">Coba kata kunci program, nomor kode, atau asnaf lain.</p>
              </div>
            ) : (
...
export default function RekonsiliasiMutasi() {
        ...
        if (targets.length > 0) {
          targets.forEach((target: any, tIdx: number) => {
            ...
            list.push({
              ...
              keterangan: target.keterangan || target.spesifikasi || prog.name,
              coaCode: target.coaCode || prog.coaCode || '',
              asnaf: target.asnaf || 'Semua',
  ...
  const penerimaanRkatOptions = useMemo<RkatOptionItem[]>(() => {
    return (rkatList || []).map(rkat => {
      ...
      return {
        ...
        keterangan: rkat.keterangan || (rkat.kategori ? `Program Pengumpulan ${rkat.kategori}` : 'Penerimaan ZIS'),
  ...
  const creditRkatOptions = useMemo<RkatOptionItem[]>(() => {
    const options: RkatOptionItem[] = [];
    ...
    rkatPenyaluranList.forEach(act => {
      options.push({
        id: act.id,
        value: act.id,
        code: act.programCode || act.pilarCode || '',
        name: act.name,
        keterangan: act.keterangan || act.spesifikasi || (act.asnaf ? `Asnaf ${act.asnaf}` : '') || act.pilarName,
        categoryOrPilar: act.pilarName || 'Penyaluran',
    ...
    // 2. Add Operasional with Auto COA Cascading / Fallback
    rkatOperasionalList.forEach(item => {
      let firstCoa = item.coa_codes ? item.coa_codes.split(',')[0].trim() : '';

      if (!firstCoa && coas && coas.length > 0) {
        const itemNamaLower = (item.nama || '').toLowerCase();
        const matched = coas.find(c => c.coa_code.startsWith('52') && (c.nama_akun.toLowerCase().includes(itemNamaLower) || itemNamaLower.includes(c.nama_akun.toLowerCase())))
          || coas.find(c => c.coa_code === '5210101' || c.coa_code.startsWith('52'));
      ...
      options.push({
        id: item.id,
        value: item.id,
        code: item.no || item.id || '',
        name: item.nama,
        keterangan: item.keterangan || item.pos_anggaran || item.deskripsi || 'Beban Operasional BAZNAS',
    ...
    return options;
  }, [rkatPenyaluranList, rkatOperasionalList, coas]);
...
'success'
 ...
 showToast(errMsg,'error');
  ...
  const handleRkatSelectPenerimaan = (rkatId: string, itemObj?: RkatOptionItem) => {
    ...
    const rkat = itemObj || penerimaanRkatOptions.find(r => r.value === rkatId);
    if (rkat && rkat.coaCode) {
      setFormCoaCode(rkat.coaCode);
    }
  };

  const handleRkatSelectPenyaluran = (rkatId: string, itemObj?: RkatOptionItem) => {
    setFormRkatId(rkatId);
    if (!rkatId) {
      setFormCoaCode('');
      return;
    }
    const matched = itemObj || creditRkatOptions.find(o => o.value === rkatId);
...
"fixed top-6 right-6 z-50 px-5 py-4 rounded-2xl shadow-xl flex items-center gap-3 border font-semibold text-xs",
 ...
 <span className="text-primary font-bold shrink-0">Rekonsiliasi Mutasi</span>
 </nav>
 ...
 <div>
 <p className="text-slate-400 text-[10px] font-black">
 ...
 className={`flex-1 sm:flex-initial px-6 py-4 text-xs font-black transition-all border-b-2 flex items-center justify-center gap-2 ${activeTab ==='PENERIMAAN'
 ...
 className={`flex-1 sm:flex-initial px-6 py-4 text-xs font-black transition-all border-b-2 flex items-center justify-center gap-2 ${activeTab ==='PENYALURAN'
 ?'border-primary text-primary bg-white'
 :'border-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-100/30'
 }`}
 >
 ...
 <span className="text-[10px] font-black text-slate-400 shrink-0 hidden sm:inline">Filter Bulan:</span>
 <input
 type="month"
 value={monthlyFilter}
 ...
 <span className="text-[10px] font-black bg-slate-100 text-slate-600 px-3 py-1.5 rounded-md border border-slate-200">
 ...
 className="px-4 py-2 bg-primary text-white rounded-xl text-xs font-black shadow-lg shadow-primary/20 hover:bg-primary/95 transition-all flex items-center gap-2 active:scale-95 shrink-0"
 >
 ...
 <th className="px-6 py-4 text-[10px] font-black text-slate-400">Akun Kas / Bank</th>
 <th className="px-6 py-4 text-[10px] font-black text-slate-400">Keterangan Koran / Deskripsi</th>
 <th className="px-6 py-4 text-[10px] font-black text-slate-400 text-right">
 {activeTab ==='PENERIMAAN' ?'Nominal Masuk' :'Nominal Keluar'}
 </th>
 <th className="px-6 py-4 text-[10px] font-black text-slate-400">Status / Alokasi</th>
...
'Tidak ada transaksi ditemukan'
 ...
 className="px-3.5 py-1.5 bg-primary/10 text-primary border border-primary/20 hover:bg-primary hover:text-white rounded-lg text-xs font-black transition-all active:scale-95"
 >
 Identifikasi
 </button>
 <button
 ...
 <label className="text-[10px] font-black text-slate-400">Tanggal Transaksi Bank</label>
 <input
 type="date"
 value={formTanggal}
 ...
 {/* Nominal */}
 <div className="space-y-1.5">
 <label className="text-[10px] font-black text-slate-400">Jumlah Nominal Transfer (Rp)</label>
 <input
 type="number"
 ...
 {/* Keterangan Koran */}
 <div className="space-y-1.5">
 <label className="text-[10px] font-black text-slate-400">Keterangan Mutasi Bank (Sesuai M-Banking)</label>
 <textarea
 rows={2}
 ...
 className="flex-1 py-3 text-slate-500 hover:bg-slate-50 border border-slate-200 rounded-xl text-xs font-black"
 >
 Batal
 </button>
 <button
 type="submit"
 className="flex-1 py-3 bg-primary hover:bg-primary/95 text-white rounded-xl text-xs font-black shadow-lg shadow-primary/20"
 >
 Simpan Mutasi
 </button>
 </div>
 ...
 <label className="text-[10px] font-black text-slate-400">Muzakki *</label>
 <button
 type="button"
 ...
 <p className="text-[9px] font-black text-primary">Registrasi Muzakki Instan</p>
 ...
 <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded text-slate-500">{m.kategori ||'Perorangan'}</span>
 </button>
 ))
 )}
 </div>
 ...
 Tidak ada di RKAT (Penerimaan di luar RKAT)
                ...
                <div className={`space-y-1.5 text-left transition-all duration-300 ${isOutsideRkat ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
                  <SearchableRkatDropdown
                    ...
                    onChange={(val, opt) => handleRkatSelectPenerimaan(val, opt)}
                    options={penerimaanRkatOptions}
                    placeholder="Cari program pengumpulan, nomor kode, keterangan..."
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between bg-primary/10 text-primary border border-primary/20 px-3.5 py-2.5 rounded-xl text-xs font-black">
                        <span className="flex items-center gap-2">
                          ...
                          <span>Terpilih: <span className="font-mono">{formCoaCode}</span> - {coas.find(c => c.coa_code === formCoaCode)?.nama_akun || 'Akun Penerimaan'}</span>
                        </span>
                        <button
                          type="button"
                          onClick={() => {
                      ...
                      {(() => {
                        const rkObj = penerimaanRkatOptions.find(r => r.value === formRkatId);
                        ...
                        return (
                          <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                            ...
                            {altCodes.map((cCode: string) => {
                              const cItem = coas.find(c => c.coa_code === cCode);
                              const isCur = formCoaCode === cCode;
                              return (
                                <button
                                  key={cCode}
                                  type="button"
                                  onClick={() => setFormCoaCode(cCode)}
                                  className={cn(
 ...
 className={cn(
"py-2 text-[10px] font-black rounded-lg border text-center transition-all",
 formSumberDana === tag
 ...
 selectedValue={formRkatId}
 onChange={(val, opt) => handleRkatSelectPenyaluran(val, opt)}
 options={creditRkatOptions}
 placeholder="Cari program penyaluran, operasional, kode, pilar, atau asnaf..."
 emptyLabel="-- Pilih Kegiatan RKAT Penyaluran / Operasional --"
 disabled={isOutsideRkat}
 />
 </div>
 ...
 <span className="flex items-center gap-2">
 <BookOpen className="size-4 shrink-0" />
 <span>Terpilih: <span className="font-mono">{formCoaCode}</span> - {coas.find(c => c.coa_code === formCoaCode)?.nama_akun || 'Akun Penyaluran/Beban'}</span>
 </span>
 <button
 type="button"
 onClick={() => {
 ...
 {/* Quick COA alternatives if RKAT has multiple mapped codes */}
 {(() => {
 const rkObj = creditRkatOptions.find(r => r.value === formRkatId);
 if (!rkObj || !rkObj.allCoaCodes || !rkObj.allCoaCodes.includes(',')) return null;
 const altCodes = rkObj.allCoaCodes.split(',').map((c: string) => c.trim()).filter(Boolean);
 if (altCodes.length <= 1) return null;
 ...
 {/* via Kas & Bank */}
 <div className="space-y-1.5 text-left">
 <label className="text-[10px] font-black text-slate-400">via Kas & Bank *</label>
 <input
 type="text"
 disabled
 ...
 {/* Nominal */}
 <div className="space-y-1.5 text-left">
 <label className="text-[10px] font-black text-slate-400">Nominal (Rp) *</label>
 <input
 type="text"
 disabled
 ...
 <div className="space-y-1.5">
 <label className="text-[10px] font-black text-slate-400">Metode Pembayaran *</label>
 <select
 disabled
 className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-xs text-slate-500 font-bold outline-none cursor-not-allowed"
 value="TRANSFER"
 ...
 <div className="space-y-1.5">
 <label className="text-[10px] font-black text-slate-450">Tanggal Pembayaran *</label>
 <input
 type="date"
 disabled
 className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-xs text-slate-500 font-mono font-bold outline-none cursor-not-allowed"
 ...
 <button
 type="submit"
 className="flex-1 py-3 bg-primary hover:bg-primary/95 text-white rounded-xl text-xs font-black shadow-lg shadow-primary/20 flex items-center justify-center gap-1.5"
 >
