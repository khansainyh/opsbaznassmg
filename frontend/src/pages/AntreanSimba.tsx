...
export default function AntreanSimba({ data, onUpdate }: AntreanSimbaProps) {
  ...
  // Proposal Detail & Journal Modal States
  const [detailModalProposal, setDetailModalProposal] = useState<ProposalMemo | null>(null);
  const [journalDetail, setJournalDetail] = useState<any | null>(null);
  const [loadingJournal, setLoadingJournal] = useState(false);

  useEffect(() => {
    if (detailModalProposal) {
      setLoadingJournal(true);
      axios.get(`/api/finance/proposal-journal/${detailModalProposal.id}`)
        .then(res => setJournalDetail(res.data?.data || null))
        .catch(err => {
          console.error('Error fetching proposal journal detail in AntreanSimba:', err);
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 bg-slate-50/50">
                ...
                {filteredList.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* 6. Action Button (Detail + Checklist) */}
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() => setDetailModalProposal(item)}
                            className="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all shadow-sm border border-slate-200 cursor-pointer"
                            title="Lihat Detail Transaksi & Jurnal COA"
                          >
                          ...
                          <button
                            ...
                            className="w-fit md:w-full max-w-[100px] p-2 md:py-2 md:px-3 text-xs bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-100 disabled:text-slate-400 text-white font-black rounded-xl shadow-sm hover:shadow-md transition-all flex items-center justify-center gap-1 cursor-pointer"
      ...
      {/* Modal Detail Transaksi & Jurnal SIMBA */}
      <AnimatePresence>
        {detailModalProposal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <motion.div 
              ...
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm"
              onClick={() => setDetailModalProposal(null)}
            />
            <motion.div 
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
            ...
            >
              ...
              <div className="p-6 border-b border-slate-100 flex justify-between items-center shrink-0 bg-slate-50">
                <div>
                  <h3 className="text-lg font-black text-slate-900 flex items-center gap-2">
                    ...
                    Detail Transaksi &amp; Jurnal SIMBA
                  </h3>
                  <p className="text-xs text-slate-500 font-medium mt-1">
                    No. Agenda: {detailModalProposal.agendaNo ? String(detailModalProposal.agendaNo) : '-'} | {detailModalProposal.jenisPengajuan || 'Perorangan'}
                  </p>
                </div>
                <button 
                  onClick={() => setDetailModalProposal(null)} 
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-mono font-bold text-slate-800 mt-0.5">{detailModalProposal.nik || '-'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">No. Telepon</p>
                        <p className="text-sm font-bold text-slate-800 mt-0.5">{detailModalProposal.noTelpon || '-'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Alamat Lengkap</p>
                        <p className="text-xs font-bold text-slate-700 mt-0.5">{detailModalProposal.alamat || '-'}</p>
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Kelurahan</p>
                          <p className="text-xs font-bold text-slate-700 mt-0.5">{detailModalProposal.kelurahan || '-'}</p>
                        </div>
                        <div>
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Kecamatan</p>
                          <p className="text-xs font-bold text-slate-700 mt-0.5">{detailModalProposal.kecamatan || '-'}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm font-bold text-slate-800 mt-0.5">{detailModalProposal.program || detailModalProposal.jenisPermohonan || 'Umum'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Golongan Asnaf</p>
                        <p className="text-sm font-bold text-slate-800 mt-0.5">{detailModalProposal.asnaf || 'Miskin'}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tipe Bantuan</p>
                        <p className="text-sm font-bold text-slate-800 mt-0.5">{detailModalProposal.tipeBantuan || 'Konsumtif'}</p>
                      </div>
                      <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100 mt-2">
                        ...
                        <p className="text-xl font-black text-slate-900">
                          {new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(detailModalProposal.nominal || 0)}
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-sm space-y-1">
                          ...
                          <p className="text-xs font-black text-emerald-600 font-mono pt-1">
                            {new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(journalDetail?.debit?.nominal || detailModalProposal.nominal || 0)}
                          </p>
                        </div>
                        ...
                        <div className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-sm space-y-1">
                          ...
                          <p className="text-xs font-black text-blue-600 font-mono pt-1">
                            {new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(journalDetail?.kredit?.nominal || detailModalProposal.nominal || 0)}
                          </p>
                        </div>
                      </div>
                      ...
                      {journalDetail?.realisasi && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-slate-600 bg-white p-3 rounded-xl border border-slate-200">
                          ...
                          <div>
                            <span className="font-semibold text-slate-400">NRM Mustahik:</span>{' '}
                            <span className="font-mono font-bold text-purple-700 bg-purple-50 px-1.5 py-0.5 rounded border border-purple-100">
                              {journalDetail.realisasi.nrm || detailModalProposal.mustahik?.nrm || 'Belum Terdaftar'}
                            </span>
                          </div>
                        </div>
                      )}
              ...
              <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end">
                <button 
                  onClick={() => setDetailModalProposal(null)}
                  className="px-6 py-2.5 bg-slate-200 hover:bg-slate-300 font-bold text-slate-700 rounded-xl text-xs transition-colors cursor-pointer"
                >
                  Tutup
                </button>
              </div>
