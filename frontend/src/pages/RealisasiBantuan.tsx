...
export default function RealisasiBantuan({ data, onUpdate }: RealisasiBantuanProps) {
  ...
  const handleComplete = async (id: string) => {
    try {
      // Persist the status in database as 'Selesai' to archive it safely
      await axios.put(`http://127.0.0.1:4000/api/proposals/${id}`, {
        status: 'Selesai'
      });

      const updatedData = data.map(item => 
        item.id === id ? { ...item, status: 'Selesai' as any } : item
      );
      ...
      console.error(e);
      alert('Gagal menyelesaikan realisasi: ' + (e.response?.data?.error || e.message));
    }
  };
