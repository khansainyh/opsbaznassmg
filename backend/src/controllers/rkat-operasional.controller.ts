...
export const getRkatOperasional = async (req: Request, res: Response) => {
  try {
    const list = await prisma.rkatOperasional.findMany();
    ...
    // Since Operasional is an expense/disbursement, we track debits (debit > 0)
...
export const createRkatOperasional = async (req: Request, res: Response) => {
  try {
    const { 
      no, nama, keterangan, coa_codes, volume, frekuensi, unit_cost, nilai_anggaran,
    ...
    const newItem = await prisma.rkatOperasional.create({
      data: {
        no: String(no),
        nama: String(nama),
        keterangan: keterangan !== undefined && keterangan !== null ? String(keterangan) : null,
        coa_codes: coa_codes !== undefined && coa_codes !== null ? String(coa_codes) : null,
        volume: volume ? parseInt(volume) : 1,
        frekuensi: frekuensi ? parseInt(frekuensi) : 1,
...
export const updateRkatOperasional = async (req: Request, res: Response) => {
  try {
    ...
    const updatedItem = await prisma.rkatOperasional.update({
      where: { id },
      data: {
        no: no !== undefined ? String(no) : undefined,
        nama: nama !== undefined ? String(nama) : undefined,
        keterangan: keterangan !== undefined ? (keterangan ? String(keterangan) : null) : undefined,
        coa_codes: coa_codes !== undefined ? (coa_codes ? String(coa_codes) : null) : undefined,
        volume: volume !== undefined ? parseInt(volume) : undefined,
        frekuensi: frekuensi !== undefined ? parseInt(frekuensi) : undefined,
...
export const deleteRkatOperasional = async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string;
    await prisma.rkatOperasional.delete({
      where: { id }
    });
    res.status(200).json({ status: 'success', message: 'RKAT Operasional program deleted.' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: String(error) });
  }
...
export const importRkatOperasional = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    for (const rawRow of rawData) {
      ...
      const nama = String(row.nama || row['nama program'] || row['nama kegiatan'] || row['nama_program'] || row['kegiatan'] || row['program'] || '').trim();

      if (!nama) continue;

      const keterangan = String(row.keterangan || row['keterangan kegiatan'] || row['keterangan_kegiatan'] || '').trim();
      const coaCodes = String(row.coa_codes || row['coa_codes'] || row['kode coa'] || row['kode_coa'] || row['coa'] || '').trim();
      
      const volume = parseInt(String(row.volume || '1')) || 1;
      const frekuensi = parseInt(String(row.frekuensi || '1')) || 1;
      const unitCostStr = String(row.unit_cost || row['unit cost'] || row['unit_cost'] || '0').replace(/[^0-9.-]/g, '');
      const unitCost = parseFloat(unitCostStr) || 0;
      const nilaiAnggaran = volume * frekuensi * unitCost;
      ...
      // Look for match by no and nama
      const existing = await prisma.rkatOperasional.findFirst({
        where: {
          OR: [
      ...
      if (existing) {
        await prisma.rkatOperasional.update({
        ...
        await prisma.rkatOperasional.create({
          data: {
            no: no || `O-${Date.now().toString().slice(-4)}`,
            nama,
